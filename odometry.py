...
class tbOdometry(object):
    ...
    def compute_distance_and_rotation(self, delta_traveled_angle_left_wheel, delta_traveled_angle_right_wheel):
        ...
        #reverse equations
        temp1l = vl / (2 / self.wheel_diameter)   # <=> twist.linear.x - twist.angular.z * (wheel_base / 2)
        temp1r = vr / (2 / self.wheel_diameter)   # <=> twist.linear.x + twist.angular.z * (wheel_base / 2)
        temp2 = ((temp1r - temp1l) /2) # <=> twist.angular.z * (wheel_base / 2)

        dist = (temp1l + temp1r) /2 # <=> twist.linear.x
        angle = temp2 / (self.distance_between_wheels /2) # <=> twist.angular.z
        # ============= YOUR CODE ENDS HERE! =====
        return dist,angle
